...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-gray-800 rounded-lg p-6 border border-gray-700">
            <h3 class="text-gray-400 text-sm uppercase tracking-wide mb-2">Totaal Leden</h3>
            <p class="text-4xl font-bold text-blue-400">{{ $totalMembers }}</p>
        </div>
        <div class="bg-gray-800 rounded-lg p-6 border border-gray-700">
            <h3 class="text-gray-400 text-sm uppercase tracking-wide mb-2">Actieve Wars</h3>
            <p class="text-4xl font-bold text-purple-400">{{ $activeWars }}</p>
        </div>
        <div class="bg-gray-800 rounded-lg p-6 border border-gray-700">
            <h3 class="text-gray-400 text-sm uppercase tracking-wide mb-2">Laatste Sync</h3>
            <p class="text-lg font-semibold text-green-400">
                {{ $settings->updated_at ? $settings->updated_at->diffForHumans() : 'Nooit' }}
            </p>
        </div>
    </div>
    ...
    <div class="bg-gray-800 rounded-lg p-6 border border-gray-700">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-xl font-semibold text-blue-400">Recente Ranked Wars</h2>
            <a href="/wars" class="text-blue-400 hover:text-blue-300 text-sm">Bekijk alle →</a>
        </div>
        
        @if($recentWars->isEmpty())
            <p class="text-gray-400">Geen wars gevonden.</p>
        @else
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead>
                        <tr class="text-left text-gray-400 border-b border-gray-700">
                            <th class="pb-3">Tegenstander</th>
                    ...
                    <tbody class="divide-y divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-700/50">
                            <td class="py-4">{{ $war->opponent_faction_name ?? 'Onbekend' }}</td>
                            <td class="py-4">
